    ...
    <Modal1 size="5xl" :show="showMoswalModal" @close ="closeModel" :showHeader="false">
      ...
      <template #body>
        <div class="flex px-3 py-2 text-lg font-medium justify-between ">
          <div>Motilal Oswal ({{ typeofdata }})</div>
          <div class="flex">
              <div
          ...
          <table class="w-full table table-report -mt-2 dark:text-white" v-if="showPositionsbol">
            <thead>
              <tr class="text-start">
                ...
                <th class="text-center whitespace-nowrap">QTY(B/S)</th>
...
<script setup lang="ts">
  ...
  function openSqOffModal(posData: any) {
    ...
    if (posData.sellquantity > posData.buyquantity){
        brokerPosSqOffData.value.transaction_type = "BUY"
        brokerPosSqOffData.value.quantity = posData.sellquantity - posData.buyquantity
    } else if(posData.sellquantity < posData.buyquantity){
        brokerPosSqOffData.value.transaction_type = "SELL"
        brokerPosSqOffData.value.quantity = posData.buyquantity - posData.sellquantity
    }
